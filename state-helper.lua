...
local font = {nil, nil, nil, nil, nil}
local fa_font = {nil, nil, nil, nil, nil, nil}
...
function imgui.BeforeDrawFrame()
	...
	if font[1] == nil then
		font[1] = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 15.0, nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
		font[2] = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 60.0, nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
		font[3] = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 13.0, nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
		font[4] = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf',  20.0, nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
		font[5] = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf',  40.0, nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
		font[6] = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf',  10.0, nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
		font[7] = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf',  18.0, nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic())
	end
end
...
function main()
	...
	if setting.fix_osm then
		...
		if not trand and setting.frac.org:find(u8'Больница') and not setting.int.first_start then
			...
			f:flush()
			f:close()
			table.insert(setting.fast_acc.sl, {
				send_chat = true,
				cmd = 'osm',
				pass_arg = true,
				text = u8'Мед. осмотр'
		...
		if #setting.cmd ~= 0 then
			for i = 1, #setting.cmd do
				if setting.cmd[i][1] == 'licauto' or setting.cmd[i][1] == 'licmoto' or setting.cmd[i][1] == 'licfly'
				or setting.cmd[i][1] == 'licfish' or setting.cmd[i][1] == 'licswim' or setting.cmd[i][1] == 'licgun'
				or setting.cmd[i][1] == 'lichunt' or setting.cmd[i][1] == 'licdig' or setting.cmd[i][1] == 'lictaxi'
				or setting.cmd[i][1] == 'licmec' or setting.cmd[i][1] == 'price' then
					table.insert(remove_tbl, i)
				end
			end
		end
		
		if #remove_tbl ~= 0 then
			table.sort(remove_tbl, function(a, b) return a > b end)
	...
	if setting.fix_gun then
		if setting.frac.org == u8'Центр Лицензирования' then
			...
			if #setting.cmd ~= 0 then
				for i = 1, #setting.cmd do
					if setting.cmd[i][1] == 'licgun' then
						trand = false
						break
					end
				end
	...
	if setting.fix_rank_cmd then
		...
		if setting.cmd ~= 0 then
			for i, v in ipairs(setting.cmd) do
				if v[1] == 'hl' or v[1] == 'cur' or v[1] == 'licmoto' then
					v[4] = '2'
					edit_rank_cmd(v[1], '2')
				elseif v[1] == 'mc' or v[1] == 'osm' or v[1] == 'licfish' or v[1] == 'exp' then
					v[4] = '3'
					edit_rank_cmd(v[1], '3')
				elseif v[1] == 'narko' or v[1] == 'rec' or v[1] == 'ant' or v[1] == 'licswim' then
					v[4] = '4'
					edit_rank_cmd(v[1], '4')
				elseif v[1] == 'licgun' or v[1] == 'lichunt' then
					v[4] = '5'
					edit_rank_cmd(v[1], '5')
				elseif v[1] == 'licdig' or v[1] == 'lictaxi' or v[1] == 'licmec' then
					v[4] = '6'
					edit_rank_cmd(v[1], '6')
				elseif v[1] == 'tatu' or v[1] == 'licfly' then
					v[4] = '7'
					edit_rank_cmd(v[1], '7')
				elseif v[1] == '+mute' or v[1] == '-mute' or v[1] == '+warn' or v[1] == '-warn' then
					v[4] = '8'
					edit_rank_cmd(v[1], '8')
				elseif v[1] == 'inv' or v[1] == 'uninv' or v[1] == 'rank' then
					v[4] = '9'
					edit_rank_cmd(v[1], '9')
				else
					v[4] = '1'
...
function skin.InputText(x_pos_input_text, y_pos_input_text, hint_text, arg_var_next_buf, buffer_size, input_text_size, filter_buf, saving_it)
	local tbl_per = {}
...
function window.main()
	...
	local function button_menu(text_but_menu, pos_but_menu, imvec4_icon_but_menu, icon_but_menu, pos_icon_but_menu, arg_but_menu, par_plus_stoika, text_yk)
		...
		imgui.SetCursorPos(imgui.ImVec2(pos_but_menu[1] + 7, pos_but_menu[2] + 4))
		local p = imgui.GetCursorScreenPos()
		...
		imgui.SetCursorPos(imgui.ImVec2(pos_but_menu[1], pos_but_menu[2] - 0.5))
		if imgui.Button(u8'##a2f'..text_but_menu, imgui.ImVec2(132, 33)) then param_act_but = true end
		imgui.PushFont(font[1])
		...
		imgui.SetCursorPos(imgui.ImVec2(pos_but_menu[1] + 39, pos_but_menu[2] + 8 + par_plus_stoika[2] + text_yk))
		...
		imgui.PushFont(fa_font[4])
		imgui.PushStyleColor(imgui.Col.Text, imgui.ImVec4(1.00, 1.00, 1.00, 1.00))
		imgui.SetCursorPos(imgui.ImVec2(pos_icon_but_menu[1], pos_icon_but_menu[2]))
	...
	local function menu_draw_up(text_m_d_up, meaning_f_t)
		...
		if meaning_f_t == nil then
			imgui.SetCursorPos(imgui.ImVec2(anim_menu_draw[1], 10))
		else
			imgui.SetCursorPos(imgui.ImVec2(anim_menu_draw[1], 10))
		end
	...
	if select_main_menu[1] and all_false_sel_basic then
		...
		local function drawn_button(y_p_b, flag_d_b, text_d_b, pl_text_d_b)
			...
			imgui.PushFont(font[1])
			imgui.SetCursorPos(imgui.ImVec2(56, y_p_b + 11 + pl_text_d_b))
			imgui.Text(text_d_b)
			imgui.PopFont()
		...
		local function drawn_icon_b(y_p_i, imvec4_i, icon_i, pos_i, deviation_i)
			...
			imgui.SetCursorPos(imgui.ImVec2(20, y_p_i + 5))
			local p = imgui.GetCursorScreenPos()
			...
			if pos_i ~= nil then
			imgui.PushFont(fa_font[4])
			imgui.SetCursorPos(imgui.ImVec2(pos_i[1], pos_i[2]))
			imgui.PushStyleColor(imgui.Col.Text, imgui.ImVec4(1.00, 1.00, 1.00, 1.00))
			imgui.Text(icon_i)
			imgui.PopStyleColor(1)
			imgui.PopFont()
		...
		skin.DrawFond({20, 267}, {0, 0}, {626, 1}, imgui.ImVec4(0.50, 0.50, 0.50, 0.40), 0, 0)
	...
	elseif select_main_menu[1] and not all_false_sel_basic then
		...
		elseif select_basic[10] then
			...
			elseif upd_status == 2 then
				...
				imgui.Text(u8'State Helper Free '..upd.version)
				imgui.PopFont()
